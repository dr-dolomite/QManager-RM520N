...
**The Unresolved-Condition Rule.** On a surface that lists *history*, a container fill encodes **present**
state, never past severity. A degradation gets a `warning-container` (or `destructive-container`) only while
it is still standing, meaning no later event has cancelled it; once its recovery appears, the row drops to
`bg-surface-container` and becomes quiet history. What happened is carried by the glyph and its ink, which
is a separate channel from what is currently wrong. Three reasons this is not optional. First, `severity:
"info"` in this system means *routine*, not *good*: a cell handoff or a band change is the radio working, so
tinting it `success-container` would spend a functional color decoratively and break the Functional-Color
Promise. Second, the obvious alternative, fading rows by age, greys out a still-broken condition simply
because time passed, and it is nearly invisible anyway: aged-vs-fresh separation measures 1.16:1 to 1.24:1,
and dark `success-container` against `destructive-container` measures about 1.00:1, a hue-only difference.
Third, because the glyph is then the sole visual carrier of severity and a screen reader cannot see a shape,
any surface applying this rule owes an `sr-only` severity word spoken before the label. Reference
implementation: `lib/event-presentation.ts` (the pure pairing model) and
