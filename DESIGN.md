...
carried by the glyph and its ink for as long as the row exists, plus by the container fill for as long as
the row has one. **Weight** is how much the row still deserves attention, and it does expire. A row is drawn
in its tonal container while it is **fresh or unresolved**, and once it is neither it settles onto
`bg-surface-container` keeping only its colored glyph: an hour-old recovery sits on the plain surface with
its green check still green. The OR is the entire rule. Freshness (one hour) decides the common case, and
