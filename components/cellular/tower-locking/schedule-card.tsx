...
// =============================================================================
// ScheduleCard — apply the tower lock only inside a daily window
// =============================================================================
// A peer card beside the two leg cards. It writes `config.schedule`, which the
// backend turns into TWO runtime systemd timers (apply + clear) rather than a
// `systemctl enable` — so a save can legitimately succeed at the config layer
// and still not arm, which is what `armed: false` reports and what this card
// has to say out loud.
//
// -----------------------------------------------------------------------------
      ...
      <Card className={TOWER_CARD}>
        ...
        <CardContent className={`${CARD_PAD} flex flex-col gap-4`}>
          ...
          <div className="flex flex-col gap-2">
            <Skeleton className={SKELETON_SHAPE.FIELD_LABEL} />
            <div className={DAY_CHIP.ROW}>
              {DAY_INDICES.map((d) => (
                <Skeleton key={d} className={SKELETON_SHAPE.DAY_CHIP} />
              ))}
            </div>
          </div>
        </CardContent>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 flex-col gap-1">
            ...
            <CardDescription className="text-pretty">
              {t("tower_locking.schedule.description")}
            </CardDescription>
          </div>
      ...
      <CardContent className={`${CARD_PAD} flex flex-col gap-4`}>
        ...
        >
          ...
          <motion.div
            ...
            className="bg-surface-container flex flex-wrap items-center gap-x-4 gap-y-3 rounded-field px-4 py-3"
          >
            <p className="text-sm font-semibold">
              {t("tower_locking.schedule.enable_label")}
            </p>
            <Switch
          ...
          <motion.div
            variants={staggerRowItem}
            className="grid grid-cols-1 gap-4 @md/card:grid-cols-2"
          >
            <div className="flex flex-col gap-2">
