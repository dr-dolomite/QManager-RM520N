...
- **Visual signature.** A tonal color system **derived from the mark**, on four axes that never borrow from each other. The mark is two tones of one blue, so blue is simultaneously the brand, the only action accent, and the identity of the 5G NR leg. **Radio identity**: blue for 5G NR, violet for 4G LTE. **Direction**: rose for download, cyan for upload — never a radio hue, because which way the bytes go and which radio carries them are different facts. **Spatial**: azure for antenna and layer readouts, which belong to no radio and no direction. **State**: the four functional colors, and only they ever report health. **Everything else is neutral**, and a figure with no honest hue is left neutral rather than given a decorative one. Color arrives as filled tonal containers carrying content, not as bright text sprinkled on white — but a *container* is for content and a *strong fill* is for compact emphasis, so a large tinted surface is a container and the small disc on it is the fill. Tokens, roles, and named rules live in `DESIGN.md`.
...
- **Google's Material 3 / Material You era** (Pixel Settings, Google Home, Wallet, Fi): the dominant *aesthetic* reference, **fused with a modern-simplistic register rather than followed to the letter.** M3 supplies the tonal mechanics — containers, shape hierarchy, grouped rows — and QManager takes them because they are load-bearing, not because they are Google's. Where faithful M3 would produce a busier or more ornamental surface than a modem console wants, the simpler composition wins: fewer accents per screen, quieter fills, uniform card shapes, and a strong preference for a repeated grid of equal tiles over a bespoke hero. A pattern is adopted here when it earns its place in a dense operator tool, and a pattern is dropped when it is only there because M3 does it. Three facets are adopted deliberately:
