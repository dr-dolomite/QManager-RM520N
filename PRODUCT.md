...
- **The quick check.** Mid-day glance at signal, watchdog state, data usage, recent events. Seconds, not minutes. Often on a phone or tablet beside the modem. In the tonal system this is the session color serves most directly: health should be readable from the shape and fill of a block before any number is parsed.
...
- **Tone.** Calm by default. Risk surfaces visibly (destructive fill, warning copy, explicit dialog) but the routine 90% feels quiet and confident.
- **Feel.** Expressive in transition, settled at rest. Every panel, chip state change, and chart update moves on Material's emphasized easing (300-400ms for anything that changes a container's size, shape, or fill) and ends completely at rest. Longer and more confident than a snappy corporate transition; never springy, never elastic, never rubber-banding. The one sanctioned overshoot in the product is the save check at 1.03 scale. Motion is allowed to be felt; it is never allowed to wobble on a tool people use to keep a connection alive.
...
- **Visual signature.** A tonal color system **derived from the mark**. The mark is two tones of one blue; that pair is the source color, so blue is simultaneously the brand, the only action accent, and the identity of the 5G NR leg. Around it sit two identity hues that never act (violet for the 4G LTE leg, cyan for counts and upload) and the four functional colors that only ever report state. Color arrives as large filled tonal containers carrying content, not as bright text sprinkled on white. Tokens, tonal roles, and named rules live in `DESIGN.md`.
...
- **Google's Material 3 / Material You era** (Pixel Settings, Google Home, Wallet, Fi): the dominant *aesthetic* reference. Three facets are adopted deliberately:
  1. **Tonal containers as the unit of color.** Content lives inside filled containers at tone 90 in light and tone 30-40 in dark. This is the mechanism behind the "candy" feel, and it is what makes a colorful interface still legible: the fill carries the block, the ink on that fill carries the words.
  2. **Shape as hierarchy.** Radius scales with the size and importance of a surface (28px inner tiles, 36px cards, 40px hero surfaces, full-round chips and nav), so a glance tells you what kind of thing you are looking at.
...
- **Apple's professional UI/UX** (macOS System Settings, the Pro-app inspectors in Logic, Final Cut, Xcode): the dominant *structural* reference, unchanged. Two facets remain load-bearing:
  ...
  2. **Professional density without engineer-ugliness.** Apple's pro apps prove dense expert tooling can stay legible and refined. QManager is a dense modem GUI; the density is earned with hierarchy and grouping, the way a pro-app inspector is, not dumped on the page.

**Supporting references (one facet each):**

- **Ubiquiti UniFi** (Network Controller, UniFi OS, Protect): **data density only.** Its dense pill-and-tag tables and inline status tags remain QManager's density heritage: cell scanner results, SMS inbox, log views. Note the one deliberate divergence: QManager's status indicators are now **filled tonal chips**, not UniFi-style outline-and-tint tags, because inside a tonal system an outlined badge reads as a weaker copy of the container beside it and loses legibility first in sunlight. UniFi is a density reference, not a layout or badge reference, and its varied-size hero-mosaic dashboard composition is still explicitly not taken.
- **Linear**: voice and microcopy. Restraint, precision, expert-tool register in every confirmation, error, and label.
- **Vercel dashboard**: light-and-dark parity and OKLCH-era discipline. Parity is the part that carries: dark mode is genuinely colored (tonal surfaces with real chroma, functional inks inverted), never a desaturated copy of light.
- **Grafana**: when data viz earns dense readouts. Specifically informs the signal panel, latency monitor, signal-history chart, antenna alignment meter.
...
- **QManager's own Watchdog and Alerts pages (Email / SMS)**: the in-family proof that live-service features share one consistent, learnable card shape, with settings grouped in self-contained cards and honest service-state chips, so learning one page teaches the other. The fuller **status-first column** anatomy (read-only live-status hero, then settings card, then activity log) is the *target* shape for these pages; `DESIGN.md` tracks it under "Aspirational / Not Yet Built". The honesty contracts apply today regardless: status surfaces reflect saved state, incapable controls explain themselves, tests only run against saved config.

**Atmospheric hints:**

- **Nokia FastMile 5G Gateway 7 web interface**: borrowed for its friendly-but-technical balance and the soft treatment of signal surfaces, where a strong quality readout (signal meters, alignment feedback, the aggregation chain) is genuinely the best affordance. Applied selectively on signal and antenna pages; recedes elsewhere.
...
- **Full-bleed feature layouts that use the page as the canvas.** A single feature that claims the whole viewport and scatters cards as loose visual fragments, instead of wrapping its settings inside one self-contained card (or a tidy grid of them) that the page lays out. The settings belong **inside** the card; the page only arranges the cards.
...
3. **Confidence through feedback, and interfaces that never lie.** Every action shows loading, success, or error. Async pipelines (SIM profile apply, with APN, TTL, scenario, and IMEI in sequence, or an APN save's full detach/attach cycle) show per-step state. The user is never left wondering "did that work?". State honesty extends to color and motion: a status surface reports what is actually running (saved settings, live service state) rather than the half-edited form, a released carrier stays visible and greyed rather than silently disappearing, a control that cannot currently work explains why instead of sitting there dead, and an ambient animation only loops where something is genuinely live. Trust is the first emotional goal, and it is built by never letting the UI claim something the device is not doing.
4. **Consistent in shape, not just in parts.** shadcn/ui components and design tokens are used uniformly, no one-off styles: a status chip looks the same on the cellular page as on the watchdog page. The same discipline governs **page structure**: every feature page is a page header (title plus muted description) followed by a uniform grid of self-contained cards, the way every macOS System Settings pane is the same shape. A user who learns one page has learned them all. A bespoke, hero-driven layout invented for a single screen is a consistency failure even when it looks good on its own. The unit of composition is the **card that wraps a settings group**, not the page. Within a card, tonal containers replace hairline dividers on glance surfaces, while genuine data tables keep hairline rows so density survives; that conditional is documented in `DESIGN.md` and is the one place the system deliberately has two answers. For features backed by a live service, the intended arrangement of those same cards is the **status-first column**, tracked in `DESIGN.md` under "Aspirational / Not Yet Built".
...
- **Outdoor-readable contrast.** Field technicians use this in direct sunlight on tablets and phones. All text and meaningful icons hit 4.5:1 minimum in both light and dark themes, including dense Grafana-style cards where small labels are most at risk. Validate with both themes against a bright ambient assumption, not just dark-room AA. Two consequences of the tonal system are load-bearing here: light-mode ink stays dark rather than tinting toward its hue, and a container fill never carries meaning alone, so when the fill washes out under glare the text and the icon still read.
- **Contrast is measured, not eyeballed.** Every role pairing in `DESIGN.md` was checked. The retune of `--warning` (L 0.75 to 0.585 in light mode) is a correctness fix: the previous amber measured 2.4:1 on a white card and failed AA. Any new pairing gets measured before merge.
- **Reduced motion respected.** `prefers-reduced-motion: reduce` removes movement and keeps opacity, through one global switch plus a media query on raw keyframes. Because motion durations are now longer, this matters more: the UI must remain perfectly usable, and feel intentional, with motion off.
- **Color is never the sole carrier of meaning.** Status chips always pair a semantic container with an icon (check, triangle, X, minus). Any paired or directional readout (download vs. upload) pairs distinct hues with glyphs or arrows so the meaning survives color-blindness. Green and amber converge under deuteranopia, so those two states additionally differ in container lightness, not just hue. In the sidebar, the active nav item differs from inactive ones by icon fill weight as well as container tone, so the active state survives grayscale. Charts and signal-quality indicators must remain readable in deuteranopia and protanopia simulation; any palette addition is re-verified.
...
- **Touch-usable in the field.** Interactive targets sized for gloved-adjacent tablet use; dense data surfaces may pack information tightly, but the controls that act on them never shrink below comfortable tap size. The move to pill-shaped controls raises the default control height, which helps here.
