...
- **Recent Activities now highlights only what's still wrong.** The dashboard's activity list has been redesigned in the new tonal style, and it colours a row only when that problem hasn't recovered yet, with a header chip reading **All clear** or a count of what's outstanding (each "Internet Lost" is paired with the "Internet Restored" that cancels it, so routine band changes and cell handoffs stay quiet).
