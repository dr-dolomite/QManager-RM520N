...
> ⚠️ WARNING: **a bare-var duration arbitrary is invalid CSS, not an off-scale duration.** Tailwind v4 compiles the custom property written directly in the brackets with no `var()` wrapper to a *literal* `transition-duration` declaration naming the property; the browser drops the declaration, so the transition ships as **no transition at all**. Six sites in this family carried it (`shapes.ts` ×5, `imei-settings-card.tsx` ×1) and all now wrap the property in `var()`. Verified by compiling the class with `bunx @tailwindcss/cli` and by grepping `out/_next/static/chunks/*.css` before and after. **The last two sites in the tree were in `components/local-network/ethernet-card.tsx:155,163`, a different route family; they were closed on 2026-08-31 by that page's re-authoring** — see [ethernet.md](ethernet.md) — and `scripts/test/ethernet-design-language.sh` [1] now keeps the form extinct under `components/local-network/`.
>
> ⚠️ **Do not write the wildcard spelling of the corrected form in prose here.** Tailwind v4 scans this repo's `docs/` and `scripts/` as source, so a `duration-` utility whose arbitrary value cannot be parsed as CSS — a literal `*` or an ellipsis standing in for the token name — becomes a real utility candidate — and `--duration-*` is not a parsable custom-property name, so Lightning CSS fails the whole stylesheet. `next build` drops the candidate silently and passes; `next dev` returns **500 on every page**. This warning previously contained exactly that spelling and was doing exactly that. Name the tokens individually (`-quick` / `-standard` / `-emphasized`) instead.
