...
> ⚠️ **The Active MIMO tile here is still `lte-container`, and it is on notice.** Its value is the same compound `LTE 1x2 | NR 2x4` string the Radio Information MIMO tile carries, so the violet says "LTE readout" about a figure spanning both radios — the exact reason that tile is **neutral**. Demoting it belongs to a pass on this page; it was deliberately not changed from the other page's side. See [color-system.md](color-system.md).
