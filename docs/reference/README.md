...
| [icon-system.md](icon-system.md) | Icons — the Icon-Boundary Rule (Material Symbols on the sidebar + dashboard route, lucide elsewhere), the self-hosted subset pipeline (`bun run icons:subset`, `FILL` must stay a range), the hand-synced `MaterialSymbolName` ↔ `ICONS` risk with no verifier, the inline-`fontSize` sizing gotcha, the two Network Status exceptions, and the dashboard's service-ring / identity-chip / 13px decisions |
