...
>
> The [What this debunks](#what-this-debunks) table offers a reconciliation — that the measured `Normal` reading came from a **slow-path** test (on-modem `curl`) while the reversal is claimed only for **IPA fast-path** bytes. That reconciliation is plausible and may well be right, but it is **not itself backed by a recorded probe** anywhere in this file or in the plan that introduced the map (the 2026-05-24 static-SoC-counter-orientation plan — a local working note, not tracked in the repo — which cited *this document* as its evidence). The only SDX55 orientation number ever written down here is the one that reads `Normal`. **This file therefore cannot settle the question, and neither can the plan.**
>
...
>
> **What is shipped today (Phase A, T4, 2026-08-26):** every SoC resolves to `normal`. The `SDX55` arm exists as a commented-out `case` line in `detect_orientation_from_soc()`; it stayed inert while `scripts/test/poller-data-used.sh` still existed to assert it (that harness, along with the rest of `scripts/test/`, was deleted 2026-09-03) — confirm it is still commented out with a source grep rather than a test run. This became urgent because until T4 the poller's SoC parser never matched any real device (it grepped `^Branch Name` with one space; the vendor file column-aligns with two), so the map had *never* been live in the field. Repairing the parser would have activated an unmeasured map for the first time, on fielded RM502Q-AE / RG502Q community devices.
>
