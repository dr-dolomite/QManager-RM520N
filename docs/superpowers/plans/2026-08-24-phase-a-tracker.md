...
| C3 | **T3 is the first thing to source a library into `qmanager_setup`.** The other `scripts/usr/lib/qmanager/*.sh` files still use the `[ -n "$_X_LOADED" ]` load guard, which **dies under `set -u`** — only `hw_profile.sh` and `platform.sh` were fixed. Sourcing more than those makes Q9 live. | T3 |
