...
| F4 | **RG501Q AT stack does not answer.** `at_stack_check: no OK from ATI after 3 attempts` on every install run, and the poller reports `AT device not responding`. | The AT transport likely differs on SDX55 — `/dev/smd11` may not be the right node, or `atcli_smd11` may not apply. **This plausibly blocks every Phase A/B task that reads the modem on an RG501Q**, so it should be triaged before T4 (the cut line, and the first task where a device *acts* on the profile) rather than after. | Phase A/B — needs its own recon |
