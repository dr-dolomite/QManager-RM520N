...
- **Model-specific behavior keys off `/etc/quectel-project-version`, not hardcoded assumptions.** This sentinel file is present on the RM520N-GL. Its presence (or absence) is what QManager branches on: the poller uses it to select the traffic-stats interface (`rmnet_ipa0`), the data-usage counter uses it to pick the correct byte-orientation map, and the installer uses it for firmware detection. Today this is **single-device** detection: the sentinel's *presence* is the branch condition, not its contents — the one exception is the data-usage orientation map, which reads `Branch Name`. Onboarding RG501Q-EU will require reading `Project Name:` / `Branch Name:` properly, because presence-only branching would silently apply RM520N-GL behavior to it. See [`reference/platform-matrix.md`](./reference/platform-matrix.md).
