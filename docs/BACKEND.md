...
Config: `/etc/qmanager/email_alerts.json`. Log: `/tmp/qmanager_email_log.json` (NDJSON, max 100 entries). msmtp config: `/etc/qmanager/msmtprc` (generated by `monitoring/alerts.sh` on save). Reload flag: `/tmp/qmanager_email_reload` (touched by the CGI, consumed by `alert_engine.sh`).
...
| `_ea_read_config` | Read `email_alerts.json` into `_ea_*` state variables (`_ea_enabled`, `_ea_recipient`, `_ea_threshold_minutes`, …) |
...
| `/etc/qmanager/email_alerts.json` | Email alert config (enabled, sender, recipient, app_password, threshold) |
...
| `/etc/qmanager/discord_bot.json` | Discord alert config (enabled, owner_discord_id, bot_token, threshold, autonomous_notify) |
