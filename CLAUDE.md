...
- **Typography**: Euclid Circular B is the UI typeface (`--font-sans`); JetBrains Mono (`--font-jetbrains-mono` → `font-mono`) is scoped to machine-voice surfaces per DESIGN.md's Machine-Voice Rule. No other typeface is loaded — Material Symbols is an icon font, not a voice. Both light and dark mode are first-class (OKLCH tokens).
