...
| **WAN Profile Management** | `cellular/apn.sh`, PDP contexts, the APN Management page (incl. the MBN card and the poller-fed "what the network granted" strip), the shared `apn_apply.sh` attach-cycle primitive any APN write must go through, or **any parser for `+CGCONTRDP`** — whose fields are quoted on SDX65 and bare on SDX55, so it must be comma-split and quote-agnostic | RM520N | `wan-profile-management.md`, `cellular-settings-family.md` |
