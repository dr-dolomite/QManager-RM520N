...
| ------- | ------------------------------- | ----- | --- |
| **Poller CPU profile** | Any performance work on `qmanager_poller` — the measured per-function attribution on both devices, the fact that ~all of its CPU is fork+exec overhead (`/bin/true` costs 2.4ms) rather than computation, that **htop under-reports the daemon by ~9x** because reaped children land in `cutime`/`cstime`, what the 2026-09-02 de-fork pass already took (−41%/−44%) and the ranked list of what is left, the fork-probe measurement protocol (the harness itself is deleted; the doc records how to re-measure), and the measured Go comparison (2.39ms/cycle, 1.7MB binary, `net/http` is the +3.9MB size trap) | Both | `poller-cpu-profile.md` |
...
| **Tailwind's content scan** | Any prose that quotes a utility class — a code comment or a doc sentence. **The CSS gate on `bun run package` is gone**, so read `next build`'s optimizer warnings yourself. Tailwind v4 auto-detects content and scans **every non-gitignored file**, so a class written in prose compiles into real CSS; four spellings 500 **every** route in `next dev`, and that failure latches until a cold restart | Both | `tailwind-prose-hazard.md` |
...
| **Recent Activities / Network Events** | `events.sh`, `/tmp/qmanager_events.json`, event tone/freshness, and **both** consumers: the dashboard feed and the `/monitoring` log (`components/monitoring/network-events/`). Also the producer/UI identifier contract — a producer message is a short sentence plus ONE trailing `(identifiers)` group that `splitEventMessage` strips out and redraws as mono chips, so deleting a tail deletes the only copy of that data | RM520N | `recent-activities.md` |
| **Dashboard (`/dashboard`)** | The bento shell, `home-component.tsx`, the page header + chip rail, `components/dashboard/shapes.ts`, the one-cascade entrance, the three-state contract (page carries the condition, cards go honest — never `??` a fallback tone in), or any of the nine widgets. **This surface is where `DESIGN.md` was written from**, so several of its constructions ARE the canon's worked examples and are excluded from change: the signal cards' quality treatment, Recent Activity's age-gated tone, CA's sanctioned `width` animation, the chart draw-in, the skeleton-overlay crossfade, and the three recorded Material-vs-lucide icon exceptions | RM520N | `dashboard.md` |
...
| **Connection Watchdog** | `/monitoring/watchdog`, `qmanager_watchcat`, the 4-tier recovery ladder, or the ping-cache staleness threshold — now **derived** from the cache's own `interval_sec`, never hardcoded | RM520N | `connection-watchdog.md` |
| **Connection Quality** | `qmanager_ping`, the four-leg hostname-primary probe chain, latency/jitter/loss, probe targets and thresholds, the dashboard "Internet" chip (reads `connectivity.status` — the **only** connectivity verdict), the producer's exact 13-key contract, `packet_loss_pct` / `jitter_ms` going `null` below a ten-sample floor (null means **unknown**, never healthy), or any consumer of a `/tmp/qmanager_ping.json` key — a jq `//` default turns an absent key into a truthy sentinel, never `null`, which kept the chip grey on every device for six weeks. **Frontend too** (canon pass 2026-09-06): the route's own `shapes.ts`, the four-tile status band, the numbered probe chain, the preset radiogroup, or `PRESET_LIMIT` — which mirrors `events.sh`'s preset table *and its comparison direction*, so a change to either must land in both | RM520N | `connection-quality.md` |
| **Tailscale VPN** | `/monitoring/tailscale`, `vpn/tailscale.sh`, `qmanager_tailscale_mgr`, the tiered GET (a shallow tier is the honest answer, so every field after `installed` is optional), the `/etc/qmanager/tailscale_ssh` intent flag and its `pending` state, boot enablement read from the **wants-symlink** rather than `systemctl is-enabled`, `is_installed()` deliberately not stat-ing the binaries (tailscaled resets its dir to 700), the six-member `TailscaleView` and its three page shapes, the `--accept-routes` health advisory the UI filters out on purpose, or the 1970 guard on "Last seen". ⚠️ **Never pass `--accept-routes` to `tailscale up`** — it strands the device until someone physically reboots it | RM520N | `tailscale.md` |
...
| **System Settings (`/system-settings`)** | The route INDEX only — `components/system-settings/shapes.ts` (the family's only shapes module, plus `derive.ts`), the 3-tile status band, the tri-state `timezone_applied` (absent means **applied** — test `=== false`), `formatOffset` returning `undefined` so a `!== null` test is always true, `useKnownSims().count` being `number \| null`, or the partial-envelope rule: a `200` with no `settings` / `scheduled_reboot` key is NOT an error, so both cards gate on `!data`. Four of the seven sub-routes have now taken the canon pass — **Logs, Connection Quality, System Health Check and Languages** — and each routes to its own row; the other three have not | RM520N | `system-settings.md` |
| **System Logs (`/system-settings/logs`)** | `/system-settings/logs`, `hooks/use-system-logs.ts`, `types/system-logs.ts`, or anything in `components/system-settings/logs/**` (its own `shapes.ts` + `derive.ts`, which import the family's shared names one level up and must never import from `components/monitoring/network-events/`), the frozen `system/logs.sh` contract, the dead `total` field, why `level` filtering is client-side, the row identity key, or the three-member feed state | RM520N | `logs.md` |
| **Languages (`/system-settings/languages`)** | `/system-settings/languages`, `hooks/use-language-packs.ts`, `lib/i18n/resolve-error.ts`, or anything in `components/system-settings/languages/**` (its own `shapes.ts` + `derive.ts`, which import the family's shared names one level up — the `logs/` precedent, NOT `system-health-check/`'s restate), the three-way catalog state (`manifest_error` means **unreachable/warning**, an empty manifest means **none published/neutral** — never one red alert), the radiogroup rows where the row IS the control and Remove must `stopPropagation`, `TAG_ON_TONAL` re-grounding a chip on the promoted row, or the install meter's `initial={{ width: 0 }}` — without it an unstyled fill paints a 45% install as a full track | RM520N | `languages.md` |
