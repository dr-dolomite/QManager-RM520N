...
- **Recent Activities** (dashboard event feed; `events.sh` NDJSON at `/tmp/qmanager_events.json` cap 50 → `at_cmd/fetch_events.sh` → `use-recent-activities` 10s poll keeping 20 newest-first; pure `lib/event-presentation.ts` does the one derivation the backend does not: `computeUnresolved()` pairs each degradation with the recovery that cancels it in ONE forward pass (cross-type `RESOLVED_BY`, same-type-at-`info` `SELF_RESOLVING`, everything else a one-shot that can never be unresolved, though it CAN still be tonal while fresh; a `type|severity` seen-later set makes three stacked "Internet Lost" rows light exactly one) and MUST be fed the full 20, never the sliced 5, or a recovery past the clip edge stops resolving; DESIGN.md's **Age-Gated Tone Rule** splits two axes: TONE (`EventTone` = `error`/`warning`/`success`/`routine`, severity-first then `RECOVERY_TYPES`, never expires, carried by glyph ink) vs. WEIGHT (age), so `presentEvent(event, unresolved, fresh)` draws the tonal container when `fresh || unresolved` (OR, not AND: `isFresh` = `FRESH_WINDOW_SEC` 3600s handles the common case, `unresolved` is the safety valve so age retires history but never a still-standing problem) and otherwise settles to `bg-surface-container` keeping `GLYPH_INK`; `routine` fill is deliberately ACHROMATIC `surface-container-high` because `severity:"info"` means routine not good (band change `:498`/`:537`, PCC handoff `:507`/`:547`, CA activate `:557`) and `primary-container` is dark-mode L 0.400 vs 0.300/0.320/0.325 for success/warning/destructive, i.e. a handoff would outshine an outage; aged-vs-fresh is only 1.16-1.24:1 (dark `success`/`destructive-container` ~1.00:1, hue-only) so glyph + `sr-only` severity word before the label are mandatory, and two states in one slot must never share a glyph; `error` IS emitted, only by `qmanager_watchcat` (`:468`/`:483`/`:512`/`:528`/`:596`)/`qmanager_profile_apply` (`:702`) and only on one-shot types, but `destructive-container` is now REACHABLE via freshness: an error one-shot is filled red for its first hour then settles to a red glyph on the plain surface; `VISIBLE_ROWS = 5` is the single source for `LIST_MAX_H`/`RENDER_COUNT` **and** `ActivitiesSkeleton` (a hard-coded 6 there would jump the page 68px at the skeleton handoff); `useNowSec()` ticks 30s independent of the fetch because the hook's error path never calls `setEvents`, so a sustained failure would otherwise freeze freshness under a still-rendering stale list, and `useTimeAgo` takes that ONE `nowSec` (plus a `Math.max(0,…)` clamp) so label and gate can never straddle the hour; chip hidden while loading/erroring (Saved-State Honesty), `error` split into no-data alert vs. notice-above-stale-list, key is `timestamp-type-message` because `pci_change` fires from two sites in one tick; `constants/network-events.ts` `EVENT_LABELS` kept as `t()` `defaultValue` and `formatTimeAgo` left alone, both have other live call sites) — `docs/reference/recent-activities.md`
...
- **Overview Splash** (public unauthenticated landing at `/`; `app/page.tsx` 3-state gate `public`/`checking`/`redirecting` — confirms the optimistic `qm_logged_in=1` cookie via `auth/check.sh` before forwarding to `/dashboard/`, else falls through to the splash so a stale cookie can't create a login trap; `OverviewCard` polls 3 public GET-only CGI endpoints under `scripts/www/cgi-bin/quecmanager/public/` — `overview.sh` is a strict jq allowlist projection of `/tmp/qmanager_status.json` with ZERO AT/`qcmd`, `hostname.sh` reads the KERNEL hostname from `/proc` (diverges from `settings.hostname`), `units.sh` reads temp/distance prefs without writing; hooks use plain `fetch` `credentials:"omit"` NOT `authFetch`; deliberate `logout()` → `/`, involuntary/credential-change redirects stay at `/login/`) — `docs/reference/overview-splash.md`
