...
| **Connection Quality** | `qmanager_ping`, latency/jitter/loss, probe targets and thresholds, the dashboard "Internet" chip (reads `connectivity.status` — the **only** connectivity verdict), the producer's exact 11-key contract, or any consumer of a `/tmp/qmanager_ping.json` key — a jq `//` default turns an absent key into a truthy sentinel, never `null`, which kept the chip grey on every device for six weeks | RM520N | `connection-quality.md` |
