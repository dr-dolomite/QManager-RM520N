...
| ------- | ------------------------------- | ----- | --- |
| **Poller CPU profile** | Any performance work on `qmanager_poller` — the measured per-function attribution on both devices, the fact that ~all of its CPU is fork+exec overhead (`/bin/true` costs 2.4ms) rather than computation, that **htop under-reports the daemon by ~9x** because reaped children land in `cutime`/`cstime`, what the 2026-09-02 de-fork pass already took (−41%/−44%) and the ranked list of what is left, the reusable `scripts/test/qm_fork_probe.sh` harness and its before/after protocol, and the measured Go comparison (2.39ms/cycle, 1.7MB binary, `net/http` is the +3.9MB size trap) | Both | `poller-cpu-profile.md` |
