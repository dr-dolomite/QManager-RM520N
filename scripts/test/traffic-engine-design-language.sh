...
#  [23] asserts start-alignment and bans the height lock. DESIGN.md does NOT ban
#       equal heights outright -- it says they are explicit, and conditional on
#       symmetry being a real property of the pair. It is not one here: the
#       verify card is a single footnote line when idle and a headline plus
#       three comparison bars when complete, so a lock would strand dead space
#       in whichever card had less to say, and which one that is changes as the
#       test runs. The ban is scoped to CARD_PAIR for that reason.
#
...
# -----------------------------------------------------------------------------
printf '\n[23] R2: the mode and verify cards pair without a height lock\n'
# The two cards have genuinely different content lengths -- the verify card is
# one footnote line when idle and a headline plus three comparison bars when
# complete -- so locking them to one height strands dead space in whichever has
# less to say, and which one that is changes while the test runs.
...
if [ -z "$pair_block" ]; then
    ...
    if printf '%s\n' "$pair_block" | grep -qE 'items-start'; then
        ok "CARD_PAIR aligns to the start, so each card sizes to its own content"
    else
        bad "CARD_PAIR does not align to the start -- the cards will stretch to match each other"
    fi
    if printf '%s\n' "$pair_block" | grep -qE 'h-full|items-stretch'; then
        show "$pair_block"
        bad "CARD_PAIR height-locks two cards whose content lengths differ"
    else
        ok "CARD_PAIR carries no height lock"
    fi
